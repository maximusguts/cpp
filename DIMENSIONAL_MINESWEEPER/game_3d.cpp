...
void initField(short ***field, short size, short mines) {
    for (short i = 0; i < size; i++) {
        ...
        for (short j = 0; j < size; j++) {
            field[i][j] = new short[size]; // allocate memory for each column
            for (short k = 0; k < size; k++) {
                field[i][j][k] = 0; // initialize all cells to 0
            }
        }
    }
    ...
    while (rest_mines > 0)
    {
        ...
        if (field[x][y][z] == 0) { // Check if the cell is empty
            field[x][y][z] = -1; // -1 indicates a mine
    ...
    for (short i = 0; i < size; i++) {
        for (short j = 0; j < size; j++) {
            for (short k = 0; k < size; k++) {
                if (field[i][j][k] == -1) continue; // skip mines

                for (short dx = -1; dx <= 1; dx++) {
                    for (short dy = -1; dy <= 1; dy++) {
                        for (short dz = -1; dz <= 1; dz++) {
                            if ((dx == 0 && dy == 0 && dz == 0)) continue; // skip the cell itself
                            short ni = i + dx, nj = j + dy, nk = k + dz;
                            if (ni >= 0 && ni < size && nj >= 0 && nj < size && nk >= 0 && nk < size && field[ni][nj][nk] == -1) {
                                field[i][j][k]++; // increment count for adjacent mines
                            }
                        }
                    }
                }
...
void Game3D(int userId) {
    ...
    while (!total_exit) {
        ...
        switch (choice) {
            case 1:
                size = 7; // Beginner
                mines = CountMines_byDimensions(3, size);
                break;
            case 2:
                size = 13; // Intermediate
                mines = CountMines_byDimensions(3, size);
                break;
            case 3:
                size = 20; // Expert
            ...
            case 4:
                ...
                while (!end) {
                    ...
                    cin >> mines;
                    if (mines >= degree(size, 3) / 2) {
                        if (mines >= degree(size, 3)) {
                            cout << "Number of mines cannot be greater than or equal to the size of the field." << endl;
                        } else {
                            cout << "Number of mines cannot be greater than or equal to half the size^3 of the field." << endl;
                        }
                        system("pause");
